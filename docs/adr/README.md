...
The audit trail of _why the code looks the way it does_. Each ADR pins a
...
referenced in the diff. The ADR's _Decision_ paragraph sits at level 1
(always visible); its _Alternatives considered_ + _Evaluation rubric_
sit at level 2 (`<details>`); its _Implementation pointers_ sit at
