# ADR-0018: Nominatim answers the bar search, proxied through the API
...
spaces calls at one per second through the same module-held timestamp the Deezer
...
- **Strengths:** No account of any kind. The rate limit and cache the policy demands are
  code, and the shape already exists in this repository, as the `adapter-rate-limited-fetch`
  blueprint the Deezer adapter carries.
