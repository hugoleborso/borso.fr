...
- Files (anticipated, written under `/implementation` — names not contractual, the `/implementation` agent may pick neighbouring paths):
  ...
  - `apps/pragma/api/src/auth/bootstrap.handler.ts` — `POST /api/admin/set-password` gated by *"no row exists in `app_config`"*, first-deploy seed only; refuses once a row is present.
