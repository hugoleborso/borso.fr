...
The spec's _Open questions for the designer pass → resolutions_ and Q.O.D. row "Auth model" frame the trade-off: per-user accounts (with audit) vs a single team-wide password.

## Decision

**Shared-password authentication.** A single team-wide secret, stored as an **argon2id** hash in **the application database** (`pragma.app_config`, a singleton row also carrying the 32-byte HMAC signing key for the session cookie), gates the entire app. POST `/api/auth` accepts the plaintext password, compares against the stored hash, and on success sets a **signed `HttpOnly` cookie** with `SameSite=Strict` and a 30-day TTL. The middleware validates the cookie's HMAC signature on every subsequent request. Failed attempts are rate-limited per `ip_hash` (5 attempts / 15 min). The five members share one secret, rotated by Hugo via `POST /api/admin/rotate-password` (no deploy, no CDK touch) — the rotation atomically updates both `password_hash` and `hmac_key`, which invalidates every existing session cookie. Per-user identity exists in the data model (the `Member` row carries the name and colour), but the auth layer does not bind a session to a particular member — any authenticated client is "the band". Picked over individual accounts because friction at access (paid 5×/session) dominates over attribution (which a 5-person team can resolve out-of-band). Picked argon2id (over scrypt or bcrypt) for memory-hardness + current best-practice; picked HttpOnly cookie (over a JS-readable JWT) for XSS-resistance, since pragma is single-origin and the JWT's local-introspection advantage is solvable with a non-secret `last-login` timestamp in `localStorage`. Picked DSQL storage (over AWS Secrets Manager at $0.80/month for 2 secrets, or SSM Parameter Store SecureString at $0/month) because rotation becomes an in-app endpoint instead of a CDK deploy, the surface stays one storage backend, and the threat model (5 trusted humans, no PII, no billing) does not justify the separation.
...
- **Summary:** One team-wide secret stored as an **argon2id** hash in the application DB (`pragma.app_config` singleton row, carrying `password_hash` + `hmac_key` + `rotated_at`). POST `/api/auth` reads the row, verifies via `argon2.verify`, on success sets a signed `HttpOnly; Secure; SameSite=Strict` session cookie with 30-day TTL. Subsequent requests carry the cookie automatically; the middleware re-verifies the HMAC against the same row. Rotation = `POST /api/admin/rotate-password` updates both columns atomically, invalidating every existing cookie. Rate-limited by `ip_hash` (5 attempts / 15 min).
- **Strengths:**
...
| Per-action attribution        | low    | 5 trusted humans + last-write-wins concurrency (cf. spec Q.O.D. "Concurrency"). Slack resolves disputes faster than an app audit log would. The spec's _Out of scope_ list explicitly excludes a per-user audit trail. |
...
- Spec: [`docs/features/pragma/first-features/spec/spec.md`](../features/pragma/first-features/spec/spec.md) — Q.O.D. row "Auth model", _Zero-defect strategy_ section (`ip_hash` rate-limit), and _Out of scope_ item "Per-user audit trail (shared password)".
...
- Files (anticipated, written under `/implementation` — names not contractual, the `/implementation` agent may pick neighbouring paths):
  ...
  - `apps/pragma/api/src/auth/rotate-password.handler.ts` — `POST /api/admin/rotate-password`, takes the new plaintext, hashes via argon2id, generates 32 fresh HMAC bytes, updates the singleton row atomically.
  - `apps/pragma/api/src/auth/bootstrap.handler.ts` — `POST /api/admin/set-password` gated by _"no row exists in `app_config`"_, first-deploy seed only; refuses once a row is present.
  - `apps/pragma/cdk/lib/pragma-stack.ts` — **no CDK secret wiring needed.** The DB connection string is the only secret the Lambda needs (already in scope for every other endpoint).
