...
| Q5 (persistence) | localStorage `borsouvertures.v1` | `site/state/localStorage.utils.ts` (typed `read` / `write` for the `v1` namespace, drop on schema bump); `useAppState` is itself a plain external store on `useSyncExternalStore` (zustand removed — see Pattern Coherence below) and persists by calling `localStorage.utils.ts` in its setters | unit test: malformed JSON → `read` returns `null`; older `borsouvertures.v0` payload → ignored; round-trip preserves shape |
| Pattern Coherence | One external-store pattern across the app | `useAppState`, `learnTreeMachine`, `playMachine` all expose `subscribe(listener)`, `getSnapshot()`, dispatcher methods. Components consume them uniformly via `useSyncExternalStore`. The imported `zustand` dep is dropped from `package.json`. | grep `zustand` repo-wide returns nothing; bundle drops ~1 KB (zustand) net of the ~30 LoC localStorage shim |
...
| Files UPDATE | `App.tsx` (loading panel + ModeLearnTree wiring), `ModePlay.tsx` (consume `playMachine`), `useAppState.ts` (rewritten as a plain external store on `useSyncExternalStore`, no zustand; persisted slices + treeVisualizationMode flow through `localStorage.utils.ts`), `loadOpenings.ts` (hard error path returns `Result<Opening[], LoadError>`), `types.ts` (add `commentary?` optional fields), `vite.config.ts` (glob import for commentary YAML), `package.json` (DROP `zustand`; ADD `yaml@^2.5.0`) | typecheck + build pass; `pnpm exec knip` flags no orphaned deps | `pnpm --filter @borso-app/borsouvertures run typecheck && pnpm --filter @borso-app/borsouvertures run build && grep -r 'zustand' apps/borsouvertures/site/ returns nothing` |
...
| `localStorage` is unavailable (private mode / quota exceeded / Safari throwback) | low | `localStorage.utils.ts` wraps reads / writes in try/catch and returns `null` on failure; setters in `useAppState` swallow write errors so persistence degrades silently to in-memory only. | unit test: stub `localStorage.setItem` to throw — store still functions, no `console.error` cascade |
| Re-implementing zustand poorly: re-renders cascading on every snapshot tick | medium | `useAppState.getSnapshot()` returns a `useMemo`-stabilised reference that only changes when a tracked field flips. Components select with the suggested `useSyncExternalStoreWithSelector` (from React's official `use-sync-external-store/with-selector` shim) when they only care about one field. Reach for the shim only after a measured re-render problem — don't pre-optimise. | React DevTools Profiler: changing `boardStyle` doesn't re-render `<MoveButtonList />` |
...
- [x] No defensive code for impossible cases — generation counter eliminates the "stale callback" branch by design; localStorage failures are handled at the boundary (`localStorage.utils.ts`), not inside utilities.
...
- `/state-machine` — generation-counter pattern, `useSyncExternalStore` integration, fake-timer testing, when to reach for `use-sync-external-store/with-selector`.
