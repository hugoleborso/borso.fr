...
| Q.O.D. 3 | Passkeys survive | Nothing touches `member_passkey`. `updateCredentialSecret` is the only write | A passkey registered before the recovery still authenticates after it; back-e2e case |
...
| Error cases | One message for two failures | `selectRecoverErrorMessageKey` in `login.core.ts` maps `invalid-recovery` → `auth.invalidRecovery`, `rate-limited`/429 → `auth.rateLimited`, falling back to `auth.unknownError` | `login.core.test.ts` covers each branch; the file is 100%-coverage gated |
| Changes | Eighteen test files stay untouched | `buildAuthenticatedApp` in `test/auth-utils.ts` swaps its `enrol()` call for `createCredentialForMember` plus `loginAsMember`, keeping its `AuthenticatedApp` return | `git diff --stat` shows `auth-utils.ts` changed and no other `*.controller.test.ts` outside `auth/` |
...
| Half-deleted enrolment leaves an orphan export, a dead i18n key or a dangling route | Medium | Deletion is enumerated file by file in the spec's *Files to change* | `pnpm exec knip` for exports; `pnpm --filter @borso/pragma-site typecheck` for the route and the query; a grep for `auth.enrol` for the i18n keys |
...
1. `pnpm install`.
2. `pnpm --filter @borso/pragma typecheck`.
3. `pnpm exec eslint --no-warn-ignored --max-warnings 0`.
4. `pnpm --filter @borso/pragma build`.
...
8. `pnpm --filter @borso/pragma run test` — the back-e2e suite needs the local Postgres, which `scripts/local-postgres.sh` boots from the workspace's own test script.
