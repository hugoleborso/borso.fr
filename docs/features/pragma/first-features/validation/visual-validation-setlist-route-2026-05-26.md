...
The fix is present in the source tree at `2984db2` but absent from the live preview bundle. The deep-link assertion that motivated this re-run still fails end-to-end against the URL the operator provided. This is a *deployment* gap, not an *implementation* gap — but per the visual-validator contract, FAIL is FAIL: the deep-link does not work for a user opening it today.
