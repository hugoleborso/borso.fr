...
const App = () => {
  ...
  const DesktopContent = () => {
    ...
    if (route === 'concert') {
      ...
      return (
        <ConcertDetail
          ...
          onOpenSetlist={() => setRoute('setlist')}
          memberStyle={t.memberStyle}
        />
      );
    }
    ...
    if (route === 'setlist')
      return (
        <SetlistEditor
          ...
          members={D.MEMBERS}
          instruments={D.INSTRUMENTS}
          badTransitions={D.BAD_TRANSITIONS}
          transitionComments={D.TRANSITION_COMMENTS}
          initialSetlist={D.SETLIST_SEP25}
          defaultLineup={D.defaultLineup}
          mastery={D.MASTERY}
          meanMasteryForSong={D.meanMasteryForSong}
          memberStyle={t.memberStyle}
          energyViz={t.energyViz}
          density={t.density}
    ...
    if (route === 'instr') return <InstrumentsAdmin instruments={D.INSTRUMENTS} />;
    return null;
  };
  ...
  const MobileContent = ({ embed }) => {
    ...
    if (route === 'catalog')
      ...
      body = (
        <MobileSongDetail
          ...
          members={D.MEMBERS}
          instruments={D.INSTRUMENTS}
          lineup={D.defaultLineup[song.id]}
          memberStyle={t.memberStyle}
          onBack={() => setRoute('catalog')}
          onPerf={() => setPerfSong(song)}
        />
    ...
    return (
      ...
      >
        ...
        {t.showInstall && embed && (
          <InstallPrompt
            onClose={() => setTweak('showInstall', false)}
  ...
  return (
    <>
      ...
      <TweaksPanel title="Tweaks">
        ...
        <div style={{ display: 'flex', gap: 4, padding: '4px 0' }}>
          {window.PragmaData.MEMBERS.map((m) => (
            <MemberChip
              ...
              member={m}
              style={t.memberStyle}
              members={window.PragmaData.MEMBERS}
            />
          ))}
        </div>
        ...
        />
        <TweakToggle
          label="Prompt d'install (PWA)"
          value={t.showInstall}
          onChange={(v) => setTweak('showInstall', v)}
        />
