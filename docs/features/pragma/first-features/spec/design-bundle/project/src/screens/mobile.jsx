...
const MobileCatalog = ({ songs, members, instruments, defaultLineup, memberStyle, onOpen }) => {
  ...
  return (
    <div className="mob">
      <div
        className="mob-head"
        style={{ flexDirection: 'column', alignItems: 'stretch', gap: 10, paddingBottom: 14 }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <h1 style={{ flex: 1 }}>Catalogue</h1>
          <button className="btn ghost sm" style={{ padding: 6 }}>
            <Icon name="filter" size={16} />
          </button>
          <button className="btn accent sm" style={{ padding: '6px 10px' }}>
  ...
  <div className="mob">
    <div className="mob-head" style={{ flexDirection: 'column', alignItems: 'stretch' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span className="crumb" style={{ marginBottom: 0, flex: 1 }}>
          {song.artist}
        </span>
        <button className="btn ghost sm" style={{ padding: 6 }}>
    ...
    <div className="mob-body" style={{ padding: '14px 14px 80px' }}>
      ...
      <pre
        className="chord-pre"
        style={{ margin: 0, fontSize: 12.5, padding: 14 }}
        dangerouslySetInnerHTML={{
          __html: CHORDPRO_SAMPLE.split('\n')
            .slice(0, 10)
            .join('\n')
...
const MobileSetlist = ({
  ...
  const onMove = (e) => {
    ...
    let bestIdx = stateRef.current.idx,
      bestDist = Infinity;
    els.forEach((el) => {
      ...
      if (d < bestDist) {
        bestDist = d;
        bestIdx = parseInt(el.dataset.msl);
      }
    });
          ...
          {warnings.length > 0 && (
            <>
              ...
              <span style={{ color: 'var(--warn)', display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                {warnings.length} transition{warnings.length > 1 ? 's' : ''}
              </span>
            </>
          )}
        </div>
...
const MobileSetlistRow = ({
  ...
  return (
    <div
      ...
      className={grabbing ? 'dragging' : ''}
      style={{
        background: 'var(--bg-elev)',
        border: '1px solid var(--line)',
        borderColor: grabbing ? 'var(--ink-700)' : 'var(--line)',
        borderRadius: 10,
        padding: cardMode ? '12px 12px 10px' : '10px 12px',
        ...
        overflow: 'hidden',
        userSelect: 'none',
        touchAction: mode === 'longpress' ? 'pan-y' : 'none',
      }}
    >
      {/* left energy stripe */}
      {!cardMode && (
        <div
          style={{
            ...
            bottom: 8,
            width: 3,
            background: energyColor(entry.energy),
            ...
            >
              ...
              {s.kind === 'concert' && (
                <div style={{ fontSize: 11, color: 'var(--ink-500)', marginTop: 4 }}>
                  {s.capacity} pers. · {Object.values(s.friends || {}).reduce((a, b) => a + b, 0)}{' '}
                  amis attendus
                </div>
              )}
            </div>
          ...
          {BAR_COLS.map((c) => (
            <button
              ...
              style={{
                appearance: 'none',
                border: '1px solid var(--line)',
                background: tab === c.id ? 'var(--ink-900)' : 'var(--bg-elev)',
                color: tab === c.id ? 'var(--bg)' : 'var(--ink-700)',
                flex: '0 0 auto',
                padding: '6px 10px',
                borderRadius: 999,
                fontSize: 11.5,
