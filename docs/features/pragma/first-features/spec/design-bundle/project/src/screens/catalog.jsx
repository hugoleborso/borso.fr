...
const CatalogList = ({
  ...
  const STATUSES = [
    { id: 'all', l: 'Toutes', c: songs.length },
    {
      id: 'concert_ready',
      l: 'Prêtes scène',
      c: songs.filter((s) => s.status === 'concert_ready').length,
    },
      ...
      <div className="ph">
        <div>
          ...
          <div className="ph-sub">
            25 titres, dont 5 prêts pour la scène · dernière mise à jour aujourd'hui
          </div>
        </div>
      ...
      >
        ...
        >
          {STATUSES.map((s) => (
            ...
            >
              {s.l}
              <span className="mono" style={{ color: 'var(--ink-400)', fontSize: 10.5 }}>
                {s.c}
              </span>
            </button>
          ))}
  ...
  <div className="song-card" onClick={onOpen}>
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', color: 'var(--ink-400)' }}>
        ...
        {!song.chartKind && (
          <span style={{ fontSize: 10, color: 'var(--ink-300)' }}>pas d'accord</span>
        )}
      </div>
    </div>
      ...
      <tbody>
        {songs.map((s) => (
          <tr key={s.id} onClick={() => onOpen(s.id)}>
            <td>
              <span style={{ fontFamily: 'var(--t-display)', fontStyle: 'italic', fontSize: 17 }}>
                {s.title}
              </span>
            </td>
            ...
            <td className="mono" style={{ fontSize: 12 }}>
              {s.tonalityStart}
              {s.tonalityStart !== s.tonalityEnd ? ` → ${s.tonalityEnd}` : ''}
            </td>
            <td>
              <Lineup
                lineup={defaultLineup[s.id]}
    ...
    <div className="page">
      ...
      <div className="ph" style={{ marginBottom: 8 }}>
        ...
        <div className="ph-actions">
          ...
          <button className="btn accent" onClick={onPerf}>
        ...
        <div className="col" style={{ gap: 16 }}>
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            ...
            >
              ...
              <button className="btn sm ghost" onClick={onPerf}>
            ...
            <div style={{ padding: 18 }}>
              <pre
                ...
                dangerouslySetInnerHTML={{
                  __html: CHORDPRO_SAMPLE.replace(/\[([^\]]+)\]/g, '<span class="ch">[$1]</span>'),
                }}
              />
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="col" style={{ gap: 6 }}>
              <ExtLink
                provider="spotify"
                url="spotify:track:38..."
                comment="Version studio Back to Black"
              />
              <ExtLink
                provider="youtube"
                url="https://youtu.be/..."
                comment="Live Royal Albert Hall — meilleure prise"
              />
            </div>
          </div>
        </div>
          ...
          <div className="card flat" style={{ background: 'var(--bg-sunk)', border: 'none' }}>
            ...
            <div style={{ fontSize: 12.5, color: 'var(--ink-700)' }}>
              ...
              <div className="muted" style={{ fontSize: 11 }}>
                3 fois en concert · 8 fois en répèt'
              </div>
            </div>
          </div>
        </aside>
...
const PerfMode = ({ song, onClose }) => {
  ...
  return (
    <div className="perf-mode" id="perf-scroll" style={{ fontSize: `${22 * zoom}px` }}>
      ...
      <pre
        style={{ fontFamily: 'var(--t-mono)', whiteSpace: 'pre', lineHeight: 2, margin: 0 }}
        dangerouslySetInnerHTML={{
          __html: CHORDPRO_SAMPLE.replace(/\{[^}]+\}\n/g, '').replace(
            /\[([^\]]+)\]/g,
            '<span class="ch">[$1]</span>',
          ),
        }}
      />
    </div>
