...
const ICONS = {
  ...
  bars: (
    <>
      <path d="M3 21V8m6 13V3m6 18v-9m6 9V8" />
    </>
  ),
  members: (
    <>
  ...
  cloud: (
    <>
      <path d="M7 18a5 5 0 1 1 0-10 6 6 0 0 1 11.5 1.5A4.5 4.5 0 1 1 17 18H7z" />
    </>
  ),
  cloudOff: (
    <>
...
const MemberChip = ({ member, style = 'chip', size = '', bare = false, members }) => {
  ...
  return (
    <span
      className={`mchip ${size} ${bare ? 'bare' : ''}`}
      data-style={style}
      style={{ '--mc': `var(${m.colorVar})` }}
      title={m.name}
    >
...
const Lineup = ({ lineup, members, instruments, style = 'chip', bare = true }) => {
  ...
  const items = Object.entries(lineup).map(([mid, iid]) => {
    ...
    return (
      <span
        key={mid}
        className={`mchip ${bare ? 'bare' : ''}`}
        data-style={style}
        style={{ '--mc': `var(${m.colorVar})` }}
        title={`${m.name} — ${inst ? inst.name : '—'}`}
      >
