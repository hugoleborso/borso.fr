...
const SetlistEditor = ({
  ...
  const onMove = (e) => {
    ...
    let bestIdx = dragRef.current.idx,
      bestDist = Infinity;
    els.forEach((el) => {
      ...
      if (d < bestDist) {
        bestDist = d;
        bestIdx = parseInt(el.dataset.slRow);
      }
    });
    setOverIdx(bestIdx);
  };
  ...
  const avgMastery = entries.map((e) => meanMasteryForSong(e.song)).filter((v) => v != null);
  const overallMastery = avgMastery.length
    ? avgMastery.reduce((a, b) => a + b, 0) / avgMastery.length
    : 0;
  ...
  return (
    <div className="page">
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 14 }}>
        ...
        <span className="crumb" style={{ marginBottom: 0 }}>
          Concert · 13 sept. 2025
        </span>
      </div>
...
const SetlistRow = ({
  ...
  return (
    <div
      className={`sl-row ${isDragging ? 'dragging' : ''}`}
      data-sl-row={idx}
      ref={rowRef}
    ...
    >
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div className="submeta">
          ...
          <span className="tag-mono">
            {song.tonalityStart}
            {song.tonalityStart !== song.tonalityEnd ? ` → ${song.tonalityEnd}` : ''}
          </span>
          {songMast != null && (
            <>
              <span style={{ color: 'var(--ink-300)' }}>·</span>
      ...
      <div className="no-drag" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <span className="tag-mono" style={{ minWidth: 22, textAlign: 'center' }}>
          {entry.energy ?? '—'}
        </span>
        <input
          className="rs no-drag"
          type="range"
          min="1"
          max="10"
          ...
          style={{ width: 88 }}
        />
        <button className="btn ghost sm no-drag" style={{ padding: 4 }}>
  ...
  return (
    <div
      style={{
        ...
        borderRadius: 10,
        padding: 12,
        boxShadow: '0 14px 40px rgba(26,22,18,0.16)',
        zIndex: 8,
      }}
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
        ...
        <button className="btn ghost sm" style={{ padding: 2 }} onClick={onClose}>
          ×
        </button>
      </div>
      <div style={{ fontSize: 12, lineHeight: 1.45, marginBottom: 10 }}>
        <span style={{ fontFamily: 'var(--t-display)', fontStyle: 'italic', fontSize: 14 }}>
          {a.title}
        </span>
      ...
      <textarea
        className="textarea"
        placeholder="Note de transition (visible en répèt')..."
          ...
          {entries.slice(0, 6).map((e, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
                <span
                  className="mono"
                  style={{ color: 'var(--ink-400)', width: 18, fontSize: 10.5 }}
                >
    ...
    <div className="scene" data-screen-label={`Mode scène ${safeI + 1}/${entries.length}`}>
      ...
      <div className="scene-head">
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <h1 className="ttl" style={{ marginTop: 4 }}>
            {song.title}
          </h1>
          <div className="sub">
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          <button className="scene-btn" onClick={() => setZoom((z) => Math.max(0.7, z - 0.1))}>
            A−
          </button>
          <button className="scene-btn" onClick={() => setZoom((z) => Math.min(1.8, z + 0.1))}>
            A+
          </button>
      ...
      <div className="scene-body" ref={bodyRef}>
        ...
        {song.chartKind ? (
          <pre
            style={{ fontSize: `${22 * zoom}px` }}
            dangerouslySetInnerHTML={{
              __html: CHORDPRO_PAGE.replace(/\[([^\]]+)\]/g, '<span class="ch">[$1]</span>'),
            }}
          />
        ) : (
          <div className="no-chart">
            Pas de grille d'accords saisie pour ce morceau.
            <br />
            <span
              style={{
                fontSize: 14,
            ...
            return (
              ...
              >
                <div className="pp">
                  ...
                  {idx === safeI ? 'maintenant' : idx === safeI + 1 ? 'à suivre' : ''}
                </div>
                <div className="pt">{s.title}</div>
                <div className="pa">
                  {s.artist} · {s.tonalityStart}
                </div>
              </div>
            );
          })}
