        ...
        >
          ...
          <div style={{ flex: 1, fontSize: 12.5 }}>
            <span style={{ fontWeight: 500 }}>
              {stale.length} bar{stale.length > 1 ? 's' : ''} sans nouvelles depuis +60 jours.
            </span>
...
const BarsKanban = ({ bars, setBars, members, memberStyle }) => {
  ...
  const onDrop = (e, col) => {
    ...
    setBars((prev) =>
      prev.map((b) =>
        b.id === id ? { ...b, status: col, last: new Date().toISOString().slice(0, 10) } : b,
      ),
    );
    setDraggingId(null);
    setHoverCol(null);
          ...
          return (
            ...
            >
              ...
              {items.map((b) => (
                <div
                  key={b.id}
                  className={`kcard ${draggingId === b.id ? 'dragging' : ''}`}
                  draggable
          ...
          return (
            <tr key={b.id} style={stale ? { background: 'rgba(184,132,26,0.04)' } : undefined}>
              <td>
                ...
                {stale && (
                  <span style={{ marginLeft: 8, color: 'var(--warn)', fontSize: 11 }}>● vieux</span>
                )}
              </td>
              <td>
                <span className="chip">{BAR_COLS.find((c) => c.id === b.status)?.label}</span>
              </td>
              <td className="mono" style={{ fontSize: 12 }}>
                {b.last ? fmtDate(b.last) : '—'}
