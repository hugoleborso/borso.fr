...
const Sidebar = ({ route, setRoute, offline }) => (
  <nav className="sidebar">
    <div className="logo">
      Pragma<small>ERP DU GROUPE</small>
    </div>
    {offline && (
      <div
        style={{
...
const MobileNav = ({ route, setRoute }) => (
  <div className="mob-nav">
    {MOB_NAV.map((it) => (
      <div
        key={it.id}
        className={`nb ${route.startsWith(it.id) ? 'active' : ''}`}
        onClick={() => setRoute(it.id)}
      >
...
const OfflineBanner = () => (
  <div className="banner-offline">
    ...
    <span style={{ opacity: 0.7 }}>
      — catalogue, accords et setlist du prochain concert disponibles. Toute modification est
      désactivée.
    </span>
