...
| Use case 3 (reset override) | Reset button restores form state to `props.defaultLineup`; the parent treats Reset as `{ lineupOverride: null }` regardless of current form state | NEW `LineupEditor.tsx` exposes `onReset()` prop (only when `surface='setlist-entry'`); `SetlistEntryRow.tsx` wires `onReset = () => onUpdate(entryId, { lineupOverride: null })` and closes the modal | `LineupEditor.test.tsx`: clicking Reset calls `onReset` once; visual-validation step 3.3 (override badge disappears) |
