...
function DirA_MonthStele({ month, isActive, isCurrent, onOpen }) {
  ...
  return (
    <button
      ...
      onMouseLeave={(e) => {
        if (!isActive) e.currentTarget.style.background = '#ece4d2';
      }}
    >
      ...
      <div style={{ position: 'absolute', left: 22, right: 22, bottom: 14 }}>
        ...
        >
          <span>
            {month.challenges.length} défi{month.challenges.length > 1 ? 's' : ''}
          </span>
          <span>
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {score.done % 1 === 0 ? score.done : score.done.toFixed(1)} défis sur {score.total}{' '}
          aboutis
        </div>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 24 }}>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            >
              défis aboutis · {Math.round(pct * 100)}%
            </div>
          </div>
        </div>
      </div>
