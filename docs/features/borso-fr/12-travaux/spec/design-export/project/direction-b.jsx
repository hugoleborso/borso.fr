  ...
  for (let mi = 0; mi < 12; mi++) {
    ...
    for (let w = 0; w < 5; w++) {
      ...
      if (hasContent) {
        ...
        if (
          month.challenges.some((c) => c.status === 'failed' || c.status === 'abandoned') &&
          w === month.challenges.length
        ) {
          bg = '#3a1a1a';
        } else if (intensity > 0) {
  ...
  return (
    <div style={{ borderBottom: '1px solid #2a2826' }}>
      <button
        ...
        onMouseEnter={(e) => {
          if (!expanded) e.currentTarget.style.background = '#161513';
        }}
        onMouseLeave={(e) => {
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          >
            {String(month.m).padStart(2, '0')} · {year}
          </div>
          <div
            style={{
            ...
            >
              <span
                style={{
                  fontFamily: '"IBM Plex Mono", monospace',
                  fontSize: 12,
                  color: c.status === 'done' || c.status === 'partial' ? '#7ee29a' : '#5a5852',
                  width: 14,
                }}
              >
                [
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div
            style={{ height: 4, background: '#2a2826', position: 'relative', overflow: 'hidden' }}
          >
            <div
              style={{
                position: 'absolute',
          ...
          >
            <span>
              {score.done % 1 === 0 ? score.done : score.done.toFixed(1)}/{score.total} aboutis
            </span>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
            <DirB_Stat
              label="aboutis"
              value={`${score.done % 1 === 0 ? score.done : score.done.toFixed(1)}/${score.total}`}
              accent="#7ee29a"
            />
          ...
          <div>
            ...
            >
              vue d'année — chaque colonne = un mois
            </div>
            <DirB_Heatmap
              yearData={yearData}
              currentMonth={year === today.year ? today.month : 13}
            />
          </div>
        </div>
      </div>
