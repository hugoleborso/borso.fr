...
function DirC_ImageSlot({ label, height, color }) {
  ...
  return (
    <div
      style={{
        height: height || 240,
        background: `repeating-linear-gradient(135deg, ${color || '#d6cdb8'} 0 8px, ${color === DIRC_ACCENT ? '#d44b1c' : '#cec4ad'} 8px 16px)`,
        position: 'relative',
        overflow: 'hidden',
        display: 'flex',
        alignItems: 'flex-end',
...
function DirC_FeaturedMonth({ month, year }) {
  ...
  return (
    ...
    >
      <div>
        <DirC_ImageSlot
          label={`photo ${month.name} · ${heroChallenge.t.slice(0, 30)}`}
          height={420}
        />
      </div>
      <div>
        ...
        >
          ...
          >
            {score.done % 1 === 0 ? score.done : score.done.toFixed(1)} sur {score.total} aboutis
          </div>
        </div>
          ...
          {month.challenges.map((c, i) => (
            <div
              ...
              style={{
                ...
                gap: 20,
                paddingBottom: 20,
                borderBottom: i < month.challenges.length - 1 ? '1px dashed #c8bea4' : 'none',
              }}
            >
              <div
                style={{
              ...
              <div>
                ...
                >
                  ...
                  >
                    {c.kind === 'daily' ? 'quotidien' : c.kind === 'count' ? 'chiffré' : 'ponctuel'}
                  </span>
                </div>
                {c.note && (
                  <div
...
function DirC_FilmstripCard({ month, year, active, isCurrent, onSelect }) {
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => {
        if (!active) {
          e.currentTarget.style.borderColor = DIRC_INK;
        }
      }}
      onMouseLeave={(e) => {
        if (!active) {
          e.currentTarget.style.borderColor = '#c8bea4';
        }
      }}
    >
      <div
        style={{
          ...
          borderBottom: `1px solid ${active ? '#3a3530' : '#c8bea4'}`,
          display: 'flex',
          justifyContent: 'space-between',
          alignItems: 'center',
        }}
      ...
      <div style={{ padding: '12px 14px 14px', flex: 1, display: 'flex', flexDirection: 'column' }}>
            ...
            <div
              ...
              style={{
                ...
                background: {
                  ...
                  partial: '#e8b76a',
                  failed: '#e89090',
                  abandoned: active ? '#5a5852' : '#bcb3a0',
                  doing: DIRC_ACCENT,
                  todo: active ? '#3a3530' : '#d6cdb8',
                }[c.status],
              }}
            />
          ))}
        ...
        >
          {score.done % 1 === 0 ? score.done : score.done.toFixed(1)}/{score.total}
        </div>
      </div>
    </button>
  );
    ...
    >
        ...
        <div style={{ display: 'flex', gap: 18, alignItems: 'baseline' }}>
          <a
            ...
            style={{
              ...
              color: DIRC_INK,
              textDecoration: 'none',
              borderBottom: `1px solid ${DIRC_INK}`,
              paddingBottom: 1,
            }}
          >
            borso<span style={{ color: DIRC_ACCENT }}>.</span>fr
          </a>
          <div
            style={{
              fontFamily: '"Space Grotesk", sans-serif',
          ...
          >
            chronique mensuelle · n° {year - 2024}
          </div>
        </div>
      ...
      >
        ...
        >
          Les douze
          <br />
          travaux<span style={{ color: DIRC_ACCENT, fontStyle: 'normal' }}>.</span>
        </h1>
        <div
          style={{
            textAlign: 'right',
        ...
        >
          ...
          >
            Douze défis par an, un par mois. Parfois plusieurs. Parfois manqués. Toujours consignés
            ici.
          </div>
        </div>
      </div>
      ...
      >
        <div>
          ...
          >
            {yearData.title}
            <span style={{ color: DIRC_ACCENT }}>.</span>
          </div>
          <div
            style={{
              fontFamily: '"Space Grotesk", sans-serif',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div
            style={{ height: 10, background: '#d6cdb8', position: 'relative', overflow: 'hidden' }}
          >
            <div
              style={{
                position: 'absolute',
          ...
          >
            <DirC_MiniStat
              label="Quotidiens"
              value={yearData.months.reduce(
                (a, m) => a + m.challenges.filter((c) => c.kind === 'daily').length,
                0,
              )}
            />
            <DirC_MiniStat
              label="Ponctuels"
              value={yearData.months.reduce(
                (a, m) => a + m.challenges.filter((c) => c.kind === 'oneshot').length,
                0,
              )}
            />
            <DirC_MiniStat
              ...
              value={yearData.months.reduce(
                (a, m) =>
                  a +
                  m.challenges.filter((c) => c.status === 'todo' || c.status === 'doing').length,
                0,
              )}
              accent
            />
      ...
      >
        ...
        <span>
          Reprise de la mesure le 1<sup>er</sup> de chaque mois.
        </span>
      </div>
    </div>
  );
