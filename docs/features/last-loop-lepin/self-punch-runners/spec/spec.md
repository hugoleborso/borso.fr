...
Le `source` default rend les rows existantes `'admin'`. Le `NOT NULL` sur `source` est la seule invariante portée en SQL ; le reste est nullable parce que la sémantique est « self-punch seulement ». L'IP n'est volontairement pas loggée (cf. _Questions, Options and Decisions_).
...
- **Visual validation** — driven by `/visual-validation`, asserts the 6 input metrics named in _Why_:
