...
function PinGate({ onAuth }) {
  ...
  function submit(e) {
    ...
    if (t >= 3) {
      setLocked(true);
      setErr('Trop de tentatives · attendre 60 s.');
      setTimeout(() => {
        setLocked(false);
        setTries(0);
        setErr('');
      ...
      setErr(
        `PIN invalide · ${3 - t} essai${3 - t > 1 ? 's' : ''} restant${3 - t > 1 ? 's' : ''}.`,
      );
    setPin('');
  }
    ...
    >
      ...
      >
        ...
        <div style={{ color: 'var(--ink-3)', fontSize: 13, marginTop: -6 }}>
          Saisir le code à 4 chiffres remis aux pointeurs.{' '}
          <span className="mute-2">Indice : 4242</span>
        </div>

        <div className="field" style={{ marginTop: 'var(--d-3)' }}>
          <label className="field-label">
            PIN <span className="en">PIN</span>
          </label>
          <input
            className="input"
            type="password"
...
function AdminConsole({ phase, setRoute, onLogout }) {
  ...
  const initialPunched = _aM(
    () => new Set(phase.inRace.filter((r) => r.progress >= 1).map((r) => r.slug)),
    [phase.key],
  );
  ...
  function punch(r) {
    ...
    if (next.has(r.slug)) {
      ...
      setBanner({
        type: 'ok',
        text: `${r.name} pointé·e · boucle ${phase.loopIdx + 1}`,
        ts: now(),
      });
      setFeed((f) =>
        [
          { ts: now(), runner: r.name, loop: phase.loopIdx + 1, by: 'orga-1', slug: r.slug },
          ...f,
        ].slice(0, 30),
      );
    }
  ...
  return (
    <div className="admin-grid" data-screen-label="04 Admin">
      <header className="admin-head">
        <div>
          ...
          <h1 className="admin-h1">
            Boucle <span className="mono">{String(phase.loopIdx + 1).padStart(2, '0')}</span> en
            cours <span className="mute-2"> · loop {phase.loopIdx + 1}</span>
          </h1>
        </div>
        <div className="admin-headtimer">
          <div className="eyebrow">Top horaire dans</div>
          <div className={`headtimer-val urgency-${urgency}`}>
            <span className="mono">{String(minutesLeft).padStart(2, '0')}</span>
            <small>min</small>
          </div>
        </div>
        <div className="row" style={{ gap: 'var(--d-2)' }}>
          <button className="btn btn-ghost btn-sm" onClick={() => setRoute('spectator')}>
            Vue spectateur ↗
          </button>
      ...
      <div className="admin-tabs">
        <button className={tab === 'punch' ? 'active' : ''} onClick={() => setTab('punch')}>
          Pointer <span className="en">PUNCH</span>
        </button>
        <button className={tab === 'dnf' ? 'active' : ''} onClick={() => setTab('dnf')}>
          DNF top horaire <span className="en">CUT-OFF</span>
        </button>
        <button className={tab === 'log' ? 'active' : ''} onClick={() => setTab('log')}>
          Journal <span className="en">LOG</span>
        </button>
        <div style={{ flex: 1 }}></div>
        <span className="mute-2 mono" style={{ fontSize: 11 }}>
          {remaining.length} restants à pointer sur la boucle
        </span>
      </div>
    ...
    return (
      <div style={{ padding: 'var(--d-6)' }}>
        ...
        <div style={{ marginTop: 8, color: 'var(--ink-2)' }}>
          Les boutons de pointage apparaîtront à 10:00, à l'instant où la première boucle démarre.
        </div>
      </div>
    );
  if (phase.key === 'finished')
    return (
      <div style={{ padding: 'var(--d-6)' }}>
        <div className="eyebrow">Course terminée</div>
        <div style={{ marginTop: 8, color: 'var(--ink-2)' }}>
          Plus rien à pointer. Voir « Journal » pour l'historique complet.
        </div>
      </div>
    );
        ...
        return (
          ...
          >
            ...
            <div className="row space-between">
              <span className="meta">
                {isPunched ? '✓ Pointé·e' : `${r.completedLoops} boucles`}
              </span>
...
function DnfWindow({ phase, punched, setPunched, setFeed, setBanner }) {
  ...
  function confirmDnf(r) {
    setFeed((f) => [
      { ts: now(), runner: r.name, loop: phase.loopIdx + 1, by: 'orga-1', dnf: true, slug: r.slug },
      ...f,
    ]);
    setBanner({
      type: 'dnf',
      text: `${r.name} marqué·e DNF — boucle ${phase.loopIdx + 1}`,
      ts: now(),
    });
  }
      ...
      <div className="row space-between" style={{ alignItems: 'baseline' }}>
        <div>
          ...
          <div style={{ fontSize: 14, color: 'var(--ink-2)', marginTop: 4 }}>
            Tous les coureurs en course n'ayant pas encore pointé. À T+0, le système suggérera ces
            DNF — vous validez ou contestez (un point manqué = sortie de course).
          </div>
        </div>
      </div>
      ...
      {notPunched.length === 0 ? (
        <div className="card" style={{ padding: 'var(--d-5)' }}>
          ...
          <div style={{ marginTop: 6, color: 'var(--ink-2)' }}>
            Aucun candidat. La boucle {phase.loopIdx + 1} est nette.
          </div>
        </div>
      ) : (
            ...
            <div key={r.slug} className="dnf-cand">
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div className="muted" style={{ fontSize: 12 }}>
                  Pace moyenne ~{r.avgLoopMin}' · dernière vue à {Math.round(r.progress * 100)}% du
                  tracé
                </div>
              </div>
              <button className="btn btn-sm">Toujours en piste</button>
              <button className="btn btn-sm btn-danger" onClick={() => confirmDnf(r)}>
                Valider DNF
              </button>
            </div>
          ))}
    ...
    <div style={{ padding: 'var(--d-4) var(--d-6)' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        <thead>
          ...
          >
            <th style={{ padding: '8px 12px', borderBottom: '1px solid var(--line-soft)' }}>
              Heure
            </th>
            <th style={{ padding: '8px 12px', borderBottom: '1px solid var(--line-soft)' }}>
              Coureur·euse
            ...
            <tr key={i} style={{ borderBottom: '1px solid var(--line-soft)' }}>
              ...
              <td style={{ padding: '10px 12px' }}>
                {p.dnf ? (
                  ...
                  <span
                    style={{ color: 'var(--warn)', fontFamily: 'var(--font-mono)', fontSize: 11 }}
                  >
                    CORRIGÉ
                  </span>
                ) : (
                  <span
                    style={{ color: 'var(--ink-2)', fontFamily: 'var(--font-mono)', fontSize: 11 }}
                  >
                    PUNCH
                  </span>
                )}
              </td>
              <td style={{ padding: '10px 12px' }} className="mono mute-2">
                {p.by}
              </td>
...
function ActionBanner({ banner, onClose }) {
  ...
  return (
    ...
    >
      ...
      <span className="mono mute-2" style={{ fontSize: 10, marginLeft: 8 }}>
        {banner.ts}
      </span>
    </div>
  );
