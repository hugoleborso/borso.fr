  ...
  return (
    <div className="su" data-screen-label="05 Setup">
      <aside className="su-side">
        <div className="eyebrow" style={{ padding: 'var(--d-4) var(--d-4) var(--d-2)' }}>
          Setup édition · admin
        </div>
        <nav className="su-steps">
          {[
            ...
            <button
              key={k}
              className={`su-step ${step === k ? 'active' : ''}`}
              onClick={() => setStep(k)}
            >
        ...
        <div style={{ padding: 'var(--d-4)', borderTop: '1px solid var(--line-soft)' }}>
          <button className="btn btn-sm" style={{ width: '100%' }} onClick={() => setRoute('home')}>
            ← Quitter
          </button>
        </div>
      </aside>
      ...
      <section className="su-main">
        ...
        {step === 'gpx' && (
          <GpxUpload onNext={() => setStep('sun')} onPrev={() => setStep('edition')} />
        )}
        {step === 'sun' && (
          <SunReadout onNext={() => setStep('roster')} onPrev={() => setStep('gpx')} />
        )}
        {step === 'roster' && (
          <Roster onNext={() => setStep('review')} onPrev={() => setStep('sun')} />
        )}
...
function GpxUpload({ onNext, onPrev }) {
  ...
  return (
    <div>
      ...
      <p className="su-section-desc">
        Importer le fichier GPX de la boucle officielle. La distance et le D+ sont calculés côté
        serveur (méthode Haversine + seuil de bruit altimétrique 3 m) et stockés en lecture seule.
      </p>
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: 14, color: 'var(--ink-2)' }}>
            Glisser un <span className="mono">.gpx</span> ici
          </div>
        ...
        <div className="card" style={{ padding: 0 }}>
          <div className="card-head">
            <div className="card-title">
              Fichier importé <span className="en">PARSED</span>
            </div>
          ...
          <div className="card-body">
            ...
            >
              <dt className="muted">Nom du fichier</dt>
              <dd className="mono">{parsed.filename}</dd>
              <dt className="muted">Points</dt>
              <dd className="mono">{parsed.points}</dd>
              <dt className="muted">Distance</dt>
              <dd className="mono">
                {parsed.distanceKm} km <span className="mute-2">(lecture seule)</span>
              </dd>
              <dt className="muted">D+</dt>
              <dd className="mono">
                {parsed.dPlusM} m <span className="mute-2">(lecture seule)</span>
              </dd>
              <dt className="muted">Départ / start</dt>
              <dd className="mono">{parsed.startLatLng}</dd>
            </dl>
            <svg
              viewBox="0 0 100 100"
              style={{ width: '100%', marginTop: 'var(--d-4)', aspectRatio: '1/1', maxHeight: 200 }}
            >
              <ellipse
                cx="50"
                cy="55"
...
function SunReadout({ onNext, onPrev }) {
  ...
  return (
    <div>
      ...
      <p className="su-section-desc">
        Sunrise et sunset sont calculés depuis la latitude et la longitude de départ, à la date du
        jour J — non modifiables (algorithme NOAA, précision ±1 min).
      </p>
      ...
      >
        <SunStat
          ...
          sub="Civil dawn"
          value={E.civilDawn}
          note="Visibilité partielle, frontale conseillée"
        />
        <SunStat
          label="Lever du soleil"
          sub="Sunrise"
          value={E.sunrise}
          note="Frontale retirable"
          emphasis
        />
        <SunStat
          ...
          sub="Sunset"
          value={E.sunset}
          note="Frontale obligatoire au top suivant"
          emphasis
        />
      ...
      <div className="card" style={{ marginTop: 'var(--d-5)', padding: 0 }}>
        <div className="card-head">
          <div className="card-title">
            Calendrier de course <span className="en">SCHEDULE</span>
          </div>
  ...
  return (
    ...
    >
      <div className="eyebrow">
        {label} · <span className="mute-2">{sub}</span>
      </div>
      <div
        style={{
        ...
        return (
          ...
          >
            <div
              ...
              style={{
                fontSize: 10,
                color: isSunset || isSunrise ? 'var(--warn)' : 'var(--ink-3)',
              }}
            >
...
function Roster({ onNext, onPrev }) {
  return (
    <div>
      ...
      <p className="su-section-desc">
        CRUD des inscrit·es. Chaque ligne devient un dossard. Liste arrêtée à 18:00 la veille
        (briefing).
      </p>
      ...
      <div className="card" style={{ marginTop: 'var(--d-3)', padding: 0 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            ...
            >
              <th style={{ padding: '10px 14px', borderBottom: '1px solid var(--line-soft)' }}>
                Bib
              </th>
              <th style={{ padding: '10px 14px', borderBottom: '1px solid var(--line-soft)' }}>
                Nom
          ...
          <tbody>
            {window.RUNNERS.slice(0, 12).map((r) => (
              <tr key={r.slug} style={{ borderBottom: '1px solid var(--line-soft)' }}>
                ...
                <td style={{ padding: '10px 14px' }} className="mono mute-2">
                  {r.slug}
                </td>
...
function Review({ onPrev, setRoute }) {
  return (
    <div>
      ...
      <p className="su-section-desc">
        Tout est prêt. Publier rend la page spectateur visible et active la console de pointage.
      </p>
      ...
      >
        <ReviewCheck ok>
          Édition créée · slug <span className="mono">lll-2026</span>
        </ReviewCheck>
        ...
        <ReviewCheck warn>
          Liste à figer à <b className="mono">2026-09-18 18:00</b> · briefing
        </ReviewCheck>
