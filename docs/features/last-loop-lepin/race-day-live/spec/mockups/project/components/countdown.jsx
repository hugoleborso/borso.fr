    ...
    <div className="card" data-screen-label="countdown">
      <div className="card-head">
        <div className="card-title">
          ...
          <span className="en">
            {isPre ? 'BEFORE START' : isDone ? 'FINISHED' : 'NEXT HOURLY TOP'}
          </span>
        </div>
        <div className="card-meta">
          {isPre
            ? `Départ ${window.EDITION.startTime}`
            : isDone
              ? '—'
              : `Boucle ${phase.loopIdx + 2}`}
        </div>
      </div>
      <div
        className="card-body"
...
function CountdownContext({ phase, r }) {
  ...
  return (
    ...
    >
      <span>
        <b style={{ color: 'var(--ink)' }}>{inField}</b> sur la boucle
      </span>
      <span className="mute-2">·</span>
      <span>
        <b style={{ color: 'var(--ink)' }}>{atCorral}</b> au corral
      </span>
      <span className="mute-2">·</span>
      <span>
        <b style={{ color: 'var(--ink)' }}>{phase.dnf.length}</b> DNF
      </span>
    </div>
  );
}
  ...
  return (
    <div style={{ textAlign: 'center', padding: 'var(--d-6) 0' }}>
      <div className="eyebrow" style={{ marginBottom: 'var(--d-3)' }}>
        Course terminée · 24 boucles · 160,9 km
      </div>
      <div
        style={{
      ...
      <div style={{ color: 'var(--ink-3)', marginTop: 'var(--d-3)' }}>
        Vainqueure · 24 boucles bouclées
      </div>
    </div>
  );
}
  ...
  return (
    <span
      ...
      style={{
        ...
        height: 88,
        background:
          'linear-gradient(180deg, var(--bg-elev-2) 0%, var(--bg-elev-2) 49%, var(--bg) 50%, var(--bg) 100%)',
  ...
  return (
    <div style={{ position: 'relative', width: 200, height: 200 }}>
      ...
      >
        <span
          className="mono"
          style={{ fontSize: 44, fontWeight: 600, letterSpacing: '-0.02em', lineHeight: 1 }}
        >
...
function BarCountdown({ mm, ss, progress }) {
  return (
    ...
    >
      <span
        className="mono"
        style={{ fontSize: 72, fontWeight: 700, letterSpacing: '-0.04em', lineHeight: 1 }}
      >
      ...
      <div
        ...
        style={{ display: 'flex', justifyContent: 'space-between', width: '100%', fontSize: 11 }}
      >
        <span>:00</span>
        <span>:15</span>
        <span>:30</span>
