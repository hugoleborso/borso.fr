  ...
  return (
    <div className="card" data-screen-label="course-map">
      ...
      <div
        ...
        style={{ display: 'grid', gridTemplateColumns: '1fr auto', minHeight: 0 }}
      >
  ...
  return (
    <div style={{ position: 'relative', width: '100%', minHeight: 320, padding: 'var(--d-4)' }}>
      ...
      >
        ...
        {/* runners */}
        {inRace.map((r) => (
          <RunnerDot key={r.slug} r={r} path={path} onPick={onPick} />
        ))}
      </svg>
      ...
      {isNight && (
        <div
          style={{
            ...
            pointerEvents: 'none',
            background:
              'radial-gradient(ellipse at 50% 50%, rgba(0,0,0,0.0) 30%, rgba(0,0,0,0.55) 90%)',
          }}
        ></div>
      )}
      {isSunset && (
...
function SunGutter({ phase }) {
  ...
  function pct(m) {
    return Math.max(0, Math.min(100, ((m - startMin) / range) * 100));
  }
  ...
  return (
    <div
      style={{
        width: 110,
        padding: 'var(--d-4) var(--d-4) var(--d-4) 0',
        borderLeft: '1px solid var(--line-soft)',
        position: 'relative',
        flexShrink: 0,
      }}
    >
      <div className="eyebrow" style={{ marginBottom: 6, paddingLeft: 'var(--d-3)' }}>
        Soleil · sun
      </div>
      <div
        style={{
    ...
    >
      <span
        style={{
          ...
          width: 14,
          height: 1,
          background: dim ? 'var(--line)' : 'var(--line)',
        }}
      ></span>
      <div style={{ lineHeight: 1.1 }}>
        <div
          style={{
            fontSize: 10,
            color: dim ? 'var(--ink-mute)' : 'var(--ink-3)',
        ...
        <div
          className="mono"
          style={{ fontSize: 10, color: dim ? 'var(--ink-mute)' : 'var(--ink-2)' }}
        >
          {sub}
        </div>
      </div>
