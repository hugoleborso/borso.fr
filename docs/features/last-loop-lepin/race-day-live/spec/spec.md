...
**Output metric (laggante, mesurée hors-CI).** _L'orga participe lui-même à la course._ Mesure binaire la première édition : au moins un membre orga court ≥ 1 boucle de l'édition tout en gérant l'événement via le site. Mesurée par retour orga la semaine suivant l'événement. Cible : **oui**. Cette métrique n'est **pas** asserted par `/visual-validation`.
...
- `dnf_validated` — événement émis quand l'orga confirme un DNF semi-auto au top horaire ; permet de tracer la friction _suggestion système ↔ confirmation humain_.
...
- **Coureur revient _pile_ au top suivant** : tolérance arrondie à la seconde côté serveur ; un coureur pointé entre 09:59:30 et 10:00:00 est valide pour la boucle 1 ; après 10:00:00 il est DNF (sauf override admin).
...
| Observabilité                                         | (a) Sentry + analytics, (b) Sentry seul, (c) rien                                                                                                                                                                                                                                                | **(a)** — Sentry frontend + 4 events analytics nommés (cf. _Input metrics_).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
Une suite vitest (gate B) qui _rejoue une édition de A à Z_ via `vi.setSystemTime()` et `app.request()` :
...
**Pas de manual sweep dans les gates.** L'orga fera un dry-run J-7 (cf. _Production strategy → smoke post-deploy_), distinct du gate de spec.
...
- _L'orga court lui-même._ Binaire : oui / non. Mesurée par retour orga la semaine suivant l'événement. Baseline papier = non (un membre orga immobilisé toute la journée). Cible v1 = oui. Bilan annuel par l'orga.
