...
- [`a-remote-image-cannot-render-in-this-sandbox.md`](./a-remote-image-cannot-render-in-this-sandbox.md) — this sandbox's Chromium rejects the agent proxy's certificate for every external https, so a cover or a CDN asset falls back to its placeholder and reads exactly like a broken feature; confirm the asset with `curl` before calling it a bug.
