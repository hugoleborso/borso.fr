...
- [`curl-and-chromium-disagree-about-an-asset-url.md`](./curl-and-chromium-disagree-about-an-asset-url.md) — a Deezer cover answers 403 to `curl` (Akamai) and renders fine in Chromium, so the command line is the unreliable witness there; also retracts the unreproducible claim that the sandbox rejects the proxy CA for every external https.
- [`a-preview-host-answers-after-its-stack-is-gone.md`](./a-preview-host-answers-after-its-stack-is-gone.md) — a merged PR's preview URL still returns 200 with the stale SPA and no API: the wildcard falls back to the shared previews CDN, which is S3-only. A 403 there says *supports only cachable requests*, not *forbidden*; read `server:` and `x-cache:` before blaming auth.
...
- [`how-a-mutation-survivor-hides.md`](./how-a-mutation-survivor-hides.md) — three cases where a test reads as sufficient and is not: a `Stryker disable` covering one line of a multi-line statement, `toContain` blind to a prepended digit, and a capture-boundary mutant that is genuinely equivalent. _2026-09-16_: and a fourth case from PR #100 — the mutant survives because the code it mutates is dead, so the fix is to delete the guard rather than to write an assertion no test could make fail.
...
- [`what-deezer-musicbrainz-and-spotify-each-carry.md`](./what-deezer-musicbrainz-and-spotify-each-carry.md) — which of the three answers without a credential, which carries an ISRC, a BPM, a key, a year or a tag, and the two fields (Spotify audio-features, AcousticBrainz) that no longer exist to plan a feature on. _2026-09-16_: Deezer answers `200` to requests it refused, stating the refusal only in the body (code `800` unknown record, code `4` quota exhausted), so a status-only adapter reports a throttle as *no results*; and Deezer indexes masters rather than songs — one room query returned six rows of one song carrying five ISRCs, so the ISRC is an exact join and an insufficient one.
...
- [`agentic-device-testing.md`](./agentic-device-testing.md) — `@swmansion/argent` drives real iOS Simulators / Android emulators / TVs; why a 375 px Chromium resize is not a phone test; what the web sandbox cannot run (no `/dev/kvm`, no Xcode); `scripts/argent.sh` is the way in, and its touch gestures do work here. _2026-08-21_: the list of what a phone does differently now names **layout** — WebKit follows the flexbox spec where Blink is lenient, so a dialog can draw correctly here and collapse to a title bar there, with no gesture involved.
- [`agent-browser-cli-quirks.md`](./agent-browser-cli-quirks.md) — `--executable-path` ignored once daemon runs; `screenshot` takes positional path, not `--output`; Chromium provisioning can fail behind proxies. _2026-08-18_: SessionStart now points the CLI at the container's own Chromium, and a `click` on a ref below the fold of a nested scroller reports success while doing nothing — scroll the element that owns the overflow, then hit-test, before calling a button dead. _2026-08-21_: there is no `resize` — widths go through `set viewport <w> <h>`, and `--width` on `open` is silently ignored; and the `eval` scope is shared across calls, so a repeated `const` fails with a SyntaxError that names JavaScript rather than the CLI. Wrap every snippet in an IIFE. _2026-09-16_: a session wedges after roughly forty drives and the symptom — a reload that never returns, a mutation stuck pending, a panel frozen mid-poll — reads as an application defect, which is how a validator records a false FAIL; `scripts/browser.sh` now counts drives per session and says so from the thirtieth. Plus four smaller traps: `--session` drops the page when a later call omits a launch flag, `batch` strips inner quotes, `find` wants `--name` after the action, and a ref goes stale within a second when the accessible name carries a live count.
...
- [`destructive-git-with-uncommitted-verification-work.md`](./destructive-git-with-uncommitted-verification-work.md) — proving an eradication works means breaking the code, and `git checkout --` then reverts the eradication with the probe. Commit first; three losses in one session. _2026-08-21_: a `-u` stash is three commits and a pathspec checkout of `stash@{0}` reads only the first, so the untracked half stays behind; a dropped stash is still recoverable through `git fsck --unreachable`.
### pnpm / package management

- [`pnpm-dev-serves-a-schema-it-never-migrated.md`](./pnpm-dev-serves-a-schema-it-never-migrated.md) — `pnpm dev` applies no migrations (only the back-e2e setup does), so a new one answers 500 with `errorMissingColumn` until that suite runs; and the dev API shares its Postgres with that suite, so seeding by hand mid-run fails an unrelated test.
...
- [`rtk-truncates-command-output.md`](./rtk-truncates-command-output.md) — a nine-line `cat` printed eight lines plus `[10 more lines]`, and the truncation reads as the file's real content; ask with `grep -c` when completeness is the question.
- [`parallel-agents-share-one-scratchpad.md`](./parallel-agents-share-one-scratchpad.md) — a fan-out gets one scratchpad, not one each, and agents pick the same obvious filename; three of eight agents on PR #81 lost a file to it, one seeing another agent's traceback from its own tool. Give each agent `<scratchpad>/<label>/`.
...
- [`edit-scripts-that-assert-then-write-lose-their-good-edits.md`](./edit-scripts-that-assert-then-write-lose-their-good-edits.md) — a script that collects replacements, asserts each, and writes once at the end discards every edit that already succeeded when a later assert fails, and reports a different edit's name while doing it.
- [`agent-browser-aborts-a-navigation-served-as-octet-stream.md`](./agent-browser-aborts-a-navigation-served-as-octet-stream.md) — `net::ERR_ABORTED` on a local navigation usually means the server answered `application/octet-stream`, which Chromium treats as a download; the proxy trap next door makes this one easy to misdiagnose.
...
- [`a-committed-generator-output-needs-a-machine-read-marker.md`](./a-committed-generator-output-needs-a-machine-read-marker.md) — the few generated files ADR-0014 keeps committed lose their "do not edit" header to `borso/no-comments`; `@generated by <path>` is the allowlisted marker, emitted by the generator itself.
- [`removing-a-comment-can-turn-a-lint-rule-red.md`](./removing-a-comment-can-turn-a-lint-rule-red.md) — `no-empty` and `no-empty-function` count a comment as content, so stripping one makes the block newly illegal; name the intent as a function instead.
...
- [`invalidatequeries-only-refetches-mounted-queries.md`](./invalidatequeries-only-refetches-mounted-queries.md) — `invalidateQueries` refetches active queries only, so a probe component that mounts the mutation without the read issues no request and passes against the invalidation bug it was written to catch. Two tests did.
