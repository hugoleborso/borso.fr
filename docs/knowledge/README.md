...
- [`what-deezer-musicbrainz-and-spotify-each-carry.md`](./what-deezer-musicbrainz-and-spotify-each-carry.md) — which of the three answers without a credential, which carries an ISRC, a BPM, a key, a year or a tag, and the two fields (Spotify audio-features, AcousticBrainz) that no longer exist to plan a feature on.
