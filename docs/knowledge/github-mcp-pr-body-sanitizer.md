...
## Angle brackets are stripped, even inside backticks

_Observed 2026-08-21, opening PR #82._ A placeholder written as
`` `@generated by <path>` `` came back as `` `@generated by ` ``. The
`<path>` was read as an HTML tag and removed, and the backticks around it
did not protect it — the sanitizer strips markup before markdown fencing is
considered.

The sentence stayed grammatical and lost its meaning, which is the worst
shape for this failure: nothing looks broken.

Applies to any angle-bracket convention a description might use — `<name>`,
`<app>`, `<slug>`, `<N>` — which is exactly how this repository writes
placeholders everywhere else, including in `CLAUDE.md` and every skill.

**Mitigation:** in a PR body, write the placeholder without brackets
(`@generated by <the generator's path>` becomes `@generated by the
generator's path`), or name a real example instead of a placeholder. Reserve
the bracket form for files in the repository, where nothing rewrites it.
