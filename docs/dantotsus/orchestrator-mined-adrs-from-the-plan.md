...
# The orchestrator was going to mine ADRs _out of_ the plan. ADRs constrain the plan.
...
That ordering is backwards. An ADR is the _reason_ a plan looks like it does — a
decision that constrains "where things land in code". Mining ADRs out of a plan
that already crystallised the decisions means the ADRs document the _result_, not
the _choice_. By the time the plan exists, the choice has been made implicitly;
the ADR is a vestigial recording, not a load-bearing artefact.

Hugo flipped the order in commit `2e64172`:

> _stage order moves from spec / plan / adrs to spec / adrs / plan. adrs
> constrain the plan rather than being mined from it._
...
the accepted ADR numbers and _must reference them in the plan_. If the spec is
thin on tech surface, the orchestrator escalates back to `/specification`
rather than guessing the choices itself.

## Root-cause chain

1. **Why** would ADRs come _after_ the plan?
   ...
   Vite", file an ADR. The plan was treated as the _source_ of decisions.
...
2. **Why** is that the wrong source?
   ...
   ADR is written _after_ the plan, it documents the projection, not the
...
3. **Why** is "spec drives ADRs" the right framing?
   The spec is where decisions are _taken_ (Q.O.D. + Changes / Types). An
   ...
   surface it _before_ the plan locks it in.
...
5. **Why** didn't this surface in design?
   ...
   the plan" without re-asking whether ADRs _constrain_ the plan or
   _describe_ it.

**Root cause:** _thought_ "ADRs document the plan's architectural choices";
_actually_ ADRs _constrain_ the plan — they're the decisions the plan
...
the plan, and the alternatives that the choice was made _against_ disappear
from the record.
...
- **`/specification`:** the spec's Q.O.D. section names alternatives + the
  ...
  _recognition_ belongs to the orchestrator.
- **`/technical-conception`:** the plan mapped Q.O.D. rows onto code. It
  ...
  machine said ADRs come _after_ the plan.
...
- `state.json` gains `adrIndex: string[]` — populated _before_ the plan
...
- `/technical-conception` receives `adrIndex` as input. The plan template
  ...
  in the index appears in at least one _Where it lands_ cell.

## Eradication (mandatory — code-level)

**Type:** Structural impossibility (level 1 — the orchestrator's state
machine _cannot_ transition `plan → implement` until `adrIndex` is
...
the _correct_ ordering (ADRs ratified pre-plan, plan references them).
