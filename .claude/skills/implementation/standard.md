...
| 3b | **When `docs/features/<app>/<slug>/spec/design-bundle/` exists, the bundle IS the visual spec — pixel-perfectly.** Before writing feature UI, the implementer (a) reads the bundle's README to confirm the contract (the bundle's own README states: *"recreate them pixel-perfectly in whatever technology makes sense for the target codebase"*), (b) reads every chat transcript under `design-bundle/chats/` to learn what the user iterated toward, (c) opens `design-bundle/project/src/styles.css` and ports its entire palette + typography stack to `apps/<app>/site/src/styles/design-tokens.css` verbatim — every named CSS variable preserved (`--bg-elev`, `--bg-sunk`, all `--ink-*` tones, all `--m-*` member-palette tones, `--accent-soft`, `--warn`, `--t-display`, `--t-ui`, `--t-mono`, etc.), (d) wires the font imports the prototype declares (typically via `@fontsource-variable/<name>` or `@fontsource/<name>` npm packages), (e) for each `design-bundle/project/src/screens/<screen>.jsx`, creates a matching React route/component under `apps/<app>/site/src/routes/<screen>/` that re-creates the prototype's DOM shape — same outer class names, same layout structure, same affordances (filter pills with counts, status chips, chart-kind icons, member chips, sparklines, drag handles, breadcrumbs, page-header subtitles), same microcopy (allowing for i18n key indirection). Token-level conformance (accent / bg matches) is NOT design fidelity — it's the floor, not the ceiling. Treating the bundle as "inspiration" is the documented failure mode: when the visual-validator opens the prototype and the implementation side-by-side, every structural divergence is a FAIL row. | pixel-faithful UI |
