...
### Open the blueprint for the layer before writing the file

Every layer this repository writes has a canonical example marked in place with a `@Blueprint` JSDoc block, and [`blueprint-index.md`](../blueprint/blueprint-index.md) maps identifier to file and line. Read the index, open the blueprint for the layer you are about to write, and copy its shape. New code that follows one carries `// @FollowsBlueprint <id>` on the line above the declaration, which is what makes adoption countable rather than assumed.

A standard states the rule and a blueprint is the working instance of it, so the two are read together: [`docs/standards/`](../../../docs/standards/README.md) for what is required, the blueprint for the shape to copy. When they disagree the standard wins and the blueprint is stale.

When the layer has no blueprint, say so and write the file against the standard, then propose the blueprint in the same change rather than leaving the next author with the same gap. [`blueprint-coverage.html`](../blueprint/blueprint-coverage.html) is where to check before assuming one exists.

The pre-commit hook and CI both run the generators in `--check` mode, so a missing tag, a duplicated identifier, a `// @FollowsBlueprint` naming a blueprint that does not exist, or a stale generated file fails the gate.
...
Conventional-commit, small focused units. Push only after typecheck, `eslint`, `prettier --check`, knip, build, `/visual-validation` (UI work), `/technical-validation` (always) all pass. Hooks must not be bypassed; their failures are the gate.
...
| 2a | Run `scripts/reports.sh blueprints`, then read `blueprint-index.md` and open the blueprint for every layer the plan's code map touches. The index is generated and not committed, so a fresh clone and a subagent worktree both start without it | the shapes to copy |
| 3 | Walk the plan's code-map table top-down, applying each change and committing in small units, marking each new file `// @FollowsBlueprint <id>` | the diff |
...
| 5 | Run pre-flight gates (typecheck, eslint, knip, build) | green gates |
...
| Writing a file for a layer without opening that layer's blueprint | The file invents a shape the repository already settled, and the reviewer spends the review re-deriving a decision instead of reading the change. |
| Leaving a new file without a `// @FollowsBlueprint` marker | The coverage grid reports the layer as unmarked, so the next author reads a real pattern as an absent one. |
