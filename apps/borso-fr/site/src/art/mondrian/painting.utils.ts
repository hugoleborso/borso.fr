...
const MULBERRY32_FIRST_XOR_SHIFT = 15;
const MULBERRY32_SECOND_XOR_SHIFT = 7;
const MULBERRY32_FINAL_XOR_SHIFT = 14;
const MULBERRY32_ODD_MULTIPLIER = 61;
...
export function mulberry32(seed: number): () => number {
  ...
  return () => {
    ...
    let mixed = state;
    mixed = Math.imul(mixed ^ (mixed >>> MULBERRY32_FIRST_XOR_SHIFT), mixed | 1);
    mixed ^=
      mixed +
      Math.imul(mixed ^ (mixed >>> MULBERRY32_SECOND_XOR_SHIFT), mixed | MULBERRY32_ODD_MULTIPLIER);
    return ((mixed ^ (mixed >>> MULBERRY32_FINAL_XOR_SHIFT)) >>> 0) / UNSIGNED_32_BIT_RANGE;
  };
}
...
const HALVES_PER_SPLIT = 2;
const MIN_SPLITTABLE_SPAN = MIN_RECT_DIMENSION * HALVES_PER_SPLIT;

function isSpanSplittable(span: number): boolean {
  return span > MIN_SPLITTABLE_SPAN;
}
...
const SPLIT_AT_GOLDEN_MINOR = 0.382;
const SPLIT_AT_MIDPOINT = 0.5;
const SPLIT_AT_GOLDEN_MAJOR = 0.618;
const SPLIT_AT_FIRST_THIRD = 0.333;
const SPLIT_AT_LAST_THIRD = 0.667;
...
const SPLIT_FRACTIONS: readonly number[] = [
  ...
  SPLIT_AT_MIDPOINT,
  SPLIT_AT_GOLDEN_MAJOR,
  SPLIT_AT_FIRST_THIRD,
  SPLIT_AT_LAST_THIRD,
  SPLIT_AT_FIRST_QUARTER,
  SPLIT_AT_LAST_QUARTER,
];
const LAST_SPLIT_FRACTION = SPLIT_AT_LAST_QUARTER;
