...
// GLSL is preserved verbatim from upstream; only the harness was retyped.
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uSpeed: { value: speed },
        uMouse: {
          value: new Float32Array([smoothMousePosition.current.x, smoothMousePosition.current.y]),
        },
    ...
    const update = (timestamp: number) => {
      ...
      if (!disableAnimation) {
        ...
        program.uniforms.uStarSpeed.value = (timestamp * 0.001 * starSpeed) / 10.0;
      }

      smoothMousePosition.current.x +=
        (targetMousePosition.current.x - smoothMousePosition.current.x) * MOUSE_LERP_FACTOR;
      smoothMousePosition.current.y +=
        (targetMousePosition.current.y - smoothMousePosition.current.y) * MOUSE_LERP_FACTOR;

      smoothMouseActive.current +=
        (targetMouseActive.current - smoothMouseActive.current) * MOUSE_LERP_FACTOR;
