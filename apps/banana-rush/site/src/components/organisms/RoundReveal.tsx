...
import { sortStoriesBySpotlight } from '@site/lib/round-spotlight.core';
...
export function RoundReveal({ game }: RoundRevealProps) {
  ...
  const stories = sortStoriesBySpotlight(buildRoundStories(game));
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col">
      ...
      <ul
        key={game.lastRound.roundNumber}
        className="grid min-h-0 flex-1 auto-rows-min grid-cols-2 content-start gap-1"
      >
        {stories.map((story) => (
          <RoundStoryRow
            key={story.playerId}
