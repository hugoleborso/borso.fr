...
export function haversineDistanceMeters(origin: LatLng, destination: LatLng): number {
  ...
  /* eslint-disable no-magic-numbers -- the halved angle, the square and the
     leading factor are the haversine formula's own notation, hav(theta) =
     sin^2(theta / 2) and d = 2R * atan2(sqrt(a), sqrt(1 - a)). A reader checks
     these lines against the formula, and a name in place of a 2 is one more
     hop between the code and the reference. */
  const haversineRoot =
