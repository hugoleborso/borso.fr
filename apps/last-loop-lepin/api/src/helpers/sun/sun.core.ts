...
function rightAscensionDegreesInTheSameQuadrantAsTheTrueLongitude(
  trueLongitudeRadians: number,
): number {
...
function computeUtcHour(
  ...
  const rightAscensionDegrees =
    rightAscensionDegreesInTheSameQuadrantAsTheTrueLongitude(trueLongitudeRadians);
 ...
 * @BlueprintDescription Lives under `helpers/sun/` because both the edition setup and the timeline read it, and takes the moment as a `date` argument so it stays pure. Its coefficients each carry the name the published algorithm gives them, and its sibling test pins the envelope the answers hold inside, so a caller can judge the accuracy from the suite rather than from a claim.
 */
