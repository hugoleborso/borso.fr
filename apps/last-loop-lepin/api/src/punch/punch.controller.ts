...
adminPunchRouter.use('*', requireAdminSession);
...
adminPunchRouter.delete('/punches/:id', async (context) => {
  try {
...
adminPunchRouter.post('/dnfs', zValidator('json', createDnfInputSchema), async (context) => {
  const input = context.req.valid('json');
