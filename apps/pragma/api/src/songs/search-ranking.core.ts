...
export function coverSearchText(hit: ExternalSongHit): string {
  return `${hit.title} ${hit.album ?? ''} ${hit.titleVersion ?? ''}`.toLowerCase();
}
...
export function popularityScore(popularity: number): number {
  if (popularity <= 0) return 0;
  return Math.round(Math.log10(popularity) * POPULARITY_WEIGHT);
}

// @FollowsBlueprint core-decision
export function scoreExternalHit(hit: ExternalSongHit, query: string): number {
  let score = popularityScore(hit.popularity);
...
export function rankExternalHits(
  ...
  return [...hits].sort((left, right) => {
    ...
    return left.deezerTrackId.localeCompare(right.deezerTrackId);
  });
}
