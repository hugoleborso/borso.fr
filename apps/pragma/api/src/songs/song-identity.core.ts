 ...
 * @BlueprintUsage Use when a record has a strong external identifier that is sometimes absent, and a duplicate must still be caught without it.
 * @BlueprintDescription Matches on the identifier when both sides carry one, because that answer is exact and no amount of spelling can defeat it. Falls back to a folded title and artist only when the identifier is missing, so a weak match can never override a strong one: two records with different identifiers are different even when their names agree, which is what a cover and its original are. The fold strips case, accents and punctuation rather than comparing raw text, because the two sides are written by two different hands.
 */
export function findCatalogueMatch<TSong extends CatalogueIdentity>(
  songs: readonly TSong[],
  candidate: CatalogueIdentity,
): TSong | null {
  if (candidate.deezerTrackId !== null) {
    return songs.find((song) => song.deezerTrackId === candidate.deezerTrackId) ?? null;
  }
