...
const RECOVERY_ERROR_KEY_BY_STATUS: ReadonlyMap<number | null, ParseKeys> = new Map([
  [TOO_MANY_ATTEMPTS_STATUS, 'auth.recoveryRateLimited'],
  [WRONG_PASSWORD_STATUS, 'auth.invalidRecovery'],
  [NOT_BOOTSTRAPPED_STATUS, 'auth.notBootstrapped'],
]);

export function selectRecoverErrorMessageKey(status: number | null): ParseKeys {
  return RECOVERY_ERROR_KEY_BY_STATUS.get(status) ?? UNKNOWN_LOGIN_ERROR_KEY;
}
