...
function isOpponentToMove(run: PlayRun): boolean {
  ...
  return (
    (run.config.side === 'white' && ply % PLIES_PER_FULL_MOVE === 1) ||
    (run.config.side === 'black' && ply % PLIES_PER_FULL_MOVE === 0)
  );
}
...
export function createPlayMachine(options: PlayMachineOptions = {}): PlayMachine {
  ...
  function undo(): void {
    const currentRun = run;
    if (!currentRun) return;
    const pliesToUndo = currentRun.config.autoOpponent ? PLIES_PER_FULL_MOVE : 1;
